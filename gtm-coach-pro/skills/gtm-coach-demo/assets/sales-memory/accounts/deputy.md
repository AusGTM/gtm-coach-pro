---
account: deputy
name: Deputy
industry: Workforce management & scheduling SaaS
segment: Mid-market
---

> **SYNTHETIC DEMO DATA.** Company names are real Australian tech companies, used for demo realism. All people, conversations, quotes, deals, and numbers below are fictional and are not affiliated with or endorsed by these companies.

# Deputy

Workforce-management & scheduling SaaS · Mid-market. Status: **open** — one active deal
(RevOps Rollout) in **negotiation**.

Fast-growing scale-up: ~60 reps across three regional teams (roughly doubled headcount in the
past year), heading into a Series C process expected to open in July 2026. Runs sales on
Salesforce; forecasting today is a manual Friday analyst roll-up layered on Clearwater
dashboards — backward-looking, with no deal-level risk scoring. Cadence is engaged to replace
that with a predictive, deal-scored forecast the CRO can defend to the board before the raise.

## Active deal
- **RevOps Rollout** — $200k, 2-year term, stage negotiation, close target 2026-06-30, owner
  jordan-rep, health yellow. Value and technical fit settled; open on commercial term length and
  legal redlines.

## Key contacts
- **Nadia Petrov** — CRO — economic buyer. Owns the board forecast.
- **Sam Whitfield** — RevOps Manager — champion. Sourced the deal, owns rollout.

## Competitive context
- **PipelineIQ** evaluated and rated weaker on deal-level risk scoring (this deal only).
- **Clearwater** remains as the BI/reporting layer, not a head-to-head competitor.
- **Status quo**: the in-house analyst spreadsheet, retiring on go-live.
