---
person: david-nguyen
name: David Nguyen
title: CFO
account: airwallex
role: economic-buyer
sentiment: negative
---

> **SYNTHETIC DEMO DATA.** Company names are real Australian tech companies, used for demo realism. All people, conversations, quotes, deals, and numbers below are fictional and are not affiliated with or endorsed by these companies.

# David Nguyen

**CFO** at Airwallex — role: **economic-buyer**, sentiment: **negative**.

The economic buyer and, ultimately, the blocker on the ~$80k forecast-eval deal. Runs a finance-led
culture where forecasting is a finance analyst deliverable, not a software category: Deepa's ~6-person
analyst team builds the monthly forecast in spreadsheets off Salesforce and a Clearwater BI layer
covers exec/board reporting. He was comfortable with that status quo and never felt the pain Rebecca
felt — "we're not flying blind here; Deepa's team hands me a number the first week of every month."
Carried a burned-tool bias too: pointed to an Anaplan seat bought ~2 years ago that nobody logged into
after onboarding as reason not to sign up for another one.

Engaged late (first joined at the 2026-02-04 demo) with his frame already formed, and never moved off
it. His two objections defined the deal and were never overcome: **"This overlaps what my analysts
already deliver."** and **"No budget allocated this cycle."** He read the platform as paying software
to do what he already pays analysts to do, saw no compelling event ("this isn't a this-quarter
decision"), and later leaned on the multi-month compliance/security review as a further reason to
defer. Because the impact was never quantified to his satisfaction, the spend never cleared his bar and
he chose the in-house status quo.

Coaching read: the deal was lost on this stakeholder. He needed a dollar-quantified impact in his own
terms and a compelling event that made inaction the expensive option — neither was delivered, and he
was reached too late to reset the frame. The open Employment Hero deal has the same shape of an
absent/blocking economic buyer; break it there earlier than we did here.

## Involvement
- 2026-02-04: **demo** — attended; voiced the "analysts already deliver" and "no budget this cycle"
  objections; called it not a this-quarter decision.
- 2026-02-25: (did not attend) — per Rebecca, had not re-engaged with the ROI case; disengaging.
- 2026-03-15: (via Rebecca) — decided not to proceed; chose the in-house analyst status quo.
