...
- [ ] **DISC-01**: User's connected Google Drive tool is discovered and bound to `~~meeting recording` via capability probe — no hardcoded tool name
...
- [ ] **DISC-04**: Each recording source records a `source_kind` (`api` | `drive_folder`) so skills branch cleanly at one point
